...
from keras.layers import Dense, LSTM, GRU, RepeatVector, TimeDistributed, Conv1D, MaxPooling1D, Dropout, Flatten
...
class mtotalModel:
    ...
    def createModel(self, type="LSTM"):
        ...
        if type == "CNN" or type == "LSTM-CNN":
            total_Y = total_Y.reshape((total_Y.shape[0], self.n_steps_out * self.n_features))
        ...
        if type == "LSTM":
            ...
            self.model.add(Conv1D(self.n_nodes_cnn, kernel_size=2, activation='relu', input_shape=(self.n_steps_in, self.n_features)))
            self.model.add(MaxPooling1D(pool_size=2))
            self.model.add(Dropout(0.2))
            self.model.add(Flatten())
            self.model.add(Dense(self.n_steps_out * self.n_features, activation='relu'))
        ...
        if type == 'LSTM' or type == 'GRU':
            ...
            train_Y = self.scaler.inverse_transform(Y_train[-1, :].reshape(-1, self.n_features))
            val_predict = self.scaler.inverse_transform(
                self.model.predict(X_val)[-1, :].reshape(-1, self.n_features))
            val_Y = self.scaler.inverse_transform(Y_val[-1, :].reshape(-1, self.n_features))
    ...
    def PredictTestSample(self, dataName, start, end):
        ...
        total = total[["carsCharging", "carsIdle", "total_kWhDelivered"]]
...
if __name__ == "__main__":
    ...
    model = mtotalModel(Total_df).createModel(type="CNN")
