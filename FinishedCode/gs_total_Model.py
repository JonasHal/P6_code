...
class totalModel:
    ...
    """
    def __init__(self, data, feature_name, n_steps_in, n_nodes, n_nodes_cnn):
        ...
        self.n_nodes_cnn = n_nodes_cnn

        self.batch_size = 25
        self.epochs = 200

    def createModel(self, type="LSTM", layers=2):
    ...
    def PredictTestSample(self, dataName, start, end):
        ...
        # Make and Invert predictions
        self.test_predict = self.mmy.inverse_transform(self.model.predict(X_test))
    ...
    def PlotLoss(self):
        ...
        plt.title(str(self.title) + ', n_in: ' + str(self.n_steps_in) +
                  ', n_nodes: ' + str(self.n_nodes) + ', n_nodes_cnn: ' + str(self.n_nodes_cnn) + ', layers: ' +
            str(self.layers) + " - " + str(self.feature_name))
        plt.xlabel('Epochs')
        plt.ylabel('Loss')
        plt.legend()
        plt.savefig(
            'tm_' + str(self.title) + '/' + str(self.feature_name) + '_n_steps_in' + str(self.n_steps_in) +
            '_n_nodes' + str(self.n_nodes) + '_n_nodes_cnn' + str(self.n_nodes_cnn) + '_layers' +
            str(self.layers))
...
if __name__ == "__main__":
    ...
    for model_type in ["LSTM", "GRU", "CNN", "LSTM-CNN"]:
            ...
            for n_steps_in in [4, 15, 50]:
                for n_nodes in [5, 50, 100]:
                    if model_type == "LSTM" or model_type == "GRU":
                        n_nodes_cnn = 32
                        for layers in [1, 2]:
                            model = totalModel(Total_df, feature_name, n_steps_in, n_nodes, n_nodes_cnn).createModel(
                                type=model_type, layers=layers)
                        ...
                        for n_nodes_cnn in [32, 64]:
                            if model_type == "LSTM-CNN":
                                ...
                                for layers in [1, 2]:
                                    model = totalModel(Total_df, feature_name, n_steps_in, n_nodes, n_nodes_cnn).createModel(type=model_type, layers=layers)
    ...
    grid_df.to_csv('../Data/gsTotal_LSTM_GRU.csv')